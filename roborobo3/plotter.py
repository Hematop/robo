...
for cn in list(df.columns.values):
	#print(cn)
	if 'inGroup' in cn:
		ig[cn.replace('inGroup','')] = (df[cn] / int(cn.replace('inGroup','')[1:5])).rolling(averaging, win_type='boxcar').mean()
	if 'perGroup' in cn:
		pg[cn.replace('perGroup','')] = df[cn].rolling(averaging, win_type='boxcar').mean()
	if 'attracted' in cn:
		at[cn.replace('attracted','')] = (df[cn] / int(cn.replace('attracted','')[1:5])).rolling(averaging, win_type='boxcar').mean()
	if 'gAttracted' in cn:
