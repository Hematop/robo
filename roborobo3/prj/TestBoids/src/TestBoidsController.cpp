...
TestBoidsController::TestBoidsController( RobotWorldModel *wm )
{
    _wm = wm;
...
TestBoidsController::~TestBoidsController()
{
}

void TestBoidsController::reset()
{
}

void TestBoidsController::step()
{
    _iteration++;
    ...
    double repulsiveRadius = gSensorRange*0;
    double attractionRadius = gSensorRange*.9; // .8 // 0.7 // if ( attractionRadius < repulsiveRadius ) then: assume attractionRadius=repulsiveRadius
    ...
    avgDeltaOrientation = avgDeltaOrientation / (nbNeighbours + 1) ;
    ...
    if ( minDistToWall < wallAvoidanceMinDist ) // (too) close to a wall?
    {
        ...
        else
        {
            ...
            _wm->_desiredTranslationalValue = speed * ( 1 - ( (double)gSensorRange - ((_wm->getCameraSensorValue(2,SENSOR_DISTANCEVALUE)+_wm->getCameraSensorValue(3,SENSOR_DISTANCEVALUE))/2) )  / (double)gSensorRange );
    ...
    else
    {
        ...
        else
        {
            if ( minDist < repulsiveRadius )
            {
                ...
                _wm->setRobotLED_colorValues(0,255,0);
                _wm->_desiredRotationalVelocity -= angleDeltaToCenterOfMass;
            }
            else
            {
                ...
                else
                {
                    ...
                    {
                        _wm->_desiredRotationalVelocity += avgDeltaOrientation;
                        _wm->setRobotLED_colorValues(255,0,0);
                    }
                    else
                    {
...
/*
void TestBoidsController::step() // THIS IS A TEMPLATE FOR PRACTICAL APPLICATION (moving like an idiot , gathering sensory information for nothing)
{
    _iteration++;
