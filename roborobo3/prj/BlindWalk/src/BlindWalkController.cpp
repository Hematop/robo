...
BlindWalkController::BlindWalkController( RobotWorldModel *__wm ) : Controller ( __wm )
{
...
void BlindWalkController::reset()
{
	// nothing to do.
}

void BlindWalkController::step()
{
    if( gWorld->getIterations() > 5 ) // ( _wm->getId() < gNbOfRobots/9 ) // the others are just replicas of the ones in the center
    {
        // SOME PARAMETERS:
        double balance = BlindWalkSharedData::gBalance; // bias towards attraction if >1. analoguous to 1/T
        double swapRate = BlindWalkSharedData::gSwapRate; // probability at each step that a node chooses random behavior
        double errorRate = BlindWalkSharedData::gErrorRate; // probability at each step that a node doesn't act according to behavior
        double acceptance = BlindWalkSharedData::gAcceptance; //.5 * (1+_isAttracted); // probability to take a neighbor into account
        int center_x = BlindWalkSharedData::gCenterX, center_y = BlindWalkSharedData::gCenterY; // position of energy-giving center
        int energyRadius = BlindWalkSharedData::gEnergyRadius; //200
        bool listeningState = BlindWalkSharedData::gListeningState;
        ...
        if(__isAttracted){
            ...
            for( int i = 0 ; i < _wm->_cameraSensorsNb ; i++)
            {
                ...
                if ( targetRawIndex >= gRobotIndexStartOffset )   // sensor ray bumped into a robot -- WARNING: one (very close) neighbor can be seen by more than one sensor (in the following, we dont care if this happen, though it is not correct to do so)
                {
                    ...
                    if( !(gEnergyLevel && listeningState) || /*dynamic_cast<BlindWalkController*>(gWorld->getRobot(targetRobotIndex)->getController())->_isAttracted){//} &&*/ dist * gEnergyMax < gWorld->getRobot(targetRobotIndex)->getWorldModel()->getEnergyLevel() ){
                        nbAttNeighbours ++;
        ...
        // (_wm->getLandmarkDistanceValue()<1)*255
            _wm->setRobotLED_colorValues(!_isAttracted*255,_isAttracted*255,_wm->getEnergyLevel());//(_wm->getEnergyLevel())*255 / gEnergyMax
  //          _wm->setRobotLED_colorValues((_wm->getGroupId()>0)*255,(_wm->getGroupId()>0)*255,(gNumberOfRobotGroups-_wm->getGroupId())*255/gNumberOfRobotGroups);
      ...
      //  monitorSensoryInformation();
        ...
        // _isAttracted = (dynamic_cast<BlindWalkController*>(gWorld->getRobot( _wm->getId() % (gNbOfRobots/9) )->getController()))->_isAttracted;
...
void BlindWalkController::moveTowards(double x, double y){ //TODO
    if(_wm->getEnergyLevel() > x*x+y*y){
        ...
        if(_wm->_desiredRotationalVelocity == 0)
            _wm->_desiredRotationalVelocity = 1 - 2*rand()%2;
        else
    ...
    _wm -> setEnergyLevel(x*x+y*y);
}

void BlindWalkController::monitorSensoryInformation()
{
