...
PopConvceController::PopConvceController( RobotWorldModel *wm )
{
    _wm = wm;
    ...
    _currentSigma = PopConvceSharedData::gSigmaRef;
    ...
    _isListening = true;
    _notListeningDelay = PopConvceSharedData::gNotListeningStateDelay;
    _listeningDelay = PopConvceSharedData::gListeningStateDelay;
...
PopConvceController::~PopConvceController()
{
    _parameters.clear();
    delete nn;
    nn = NULL;
...
void PopConvceController::step() // handles control decision and evolution (but: actual movement is done in roborobo's main loop)
{
    _iteration++;
...
std::vector<double> PopConvceController::getInputs(){
...
void PopConvceController::stepController()
{
    ...
    if ( PopConvceSharedData::gEnergyRequestOutput )
    {
...
void PopConvceController::createNN()
{
    ...
    switch ( PopConvceSharedData::gControllerType )
    {
        case 0:
        {
            // MLP
        ...
        default: // default: no controller
            std::cerr << "[ERROR] gController type unknown (value: " << PopConvceSharedData::gControllerType << ").\n";
            exit(-1);
    };
}


unsigned int PopConvceController::computeRequiredNumberOfWeights()
{
...
void PopConvceController::stepEvolution()
{
        // update energy level
    if(gEnergyLevel){
        _wm->setEnergyLevel(_wm->getEnergyLevel()-1);
        if(_wm->getEnergyLevel() <= 0)
            _wm->setAlive(false);
    }


    if( gWorld->getIterations() > 0 && gWorld->getIterations() % PopConvceSharedData::gEvaluationTime == 0 )
    {
    ...
    if ( PopConvceSharedData::gLogGenomeSnapshot && gWorld->getIterations() % ( PopConvceSharedData::gEvaluationTime * PopConvceSharedData::gSnapshotsFrequency ) == 1 )
    {
        // Logging: full genome
        std::string sLog = std::string("");
...
void PopConvceController::performVariation()
{
    if ( PopConvceSharedData::gIndividualMutationRate > rand()/RAND_MAX ) // global mutation rate (whether this genome will get any mutation or not) - default: always
    {
        switch ( PopConvceSharedData::gMutationOperator )
        {
            case 0:
                mutateUniform();
                break;
            case 1:
                ...
                break;
            case 2:
                mutateGaussian(PopConvceSharedData::gSigma); // fixed mutation rate
                break;
            default:
                std::cerr << "[ERROR] unknown variation method (gMutationOperator = " << PopConvceSharedData::gMutationOperator << ")\n";
                exit(-1);
        }
    }
}

void PopConvceController::selectRandomGenome() // if called, assume genomeList.size()>0
{
...
void PopConvceController::selectBestGenome() // if called, assume genomeList.size()>0
{
    _currentGenome = (*_genomesList.begin()).second;
    _currentSigma = _sigmaList[(*_genomesList.begin()).first];
    double fitness = _fitnessValuesList[(*_genomesList.begin()).first];
    std::map<std::pair<int,int>, std::vector<double> >::iterator it = _genomesList.begin();
    for (int i = 1; i<_genomesList.size(); i++)
    {
        it ++;
        if(_fitnessValuesList[(*it).first] > fitness){
            fitness = _fitnessValuesList[(*it).first];
            _currentGenome = (*it).second;
            _currentSigma = _sigmaList[(*it).first];
        }
    }
...
void PopConvceController::selectFirstGenome()  // if called, assume genomeList.size()>0
{
 ...
 */
bool PopConvceController::storeGenome(std::vector<double> genome, std::pair<int,int> senderId, float sigma, float fitness) // fitness is optional (default: 0)
{
    if ( !_isListening )
    {
...
void PopConvceController::mutateGaussian(float sigma) // mutate within bounds.
{
    _currentSigma = sigma;
...
void PopConvceController::mutateUniform() // mutate within bounds.
{
    for (unsigned int i = 0 ; i != _currentGenome.size() ; i++ )
    {
...
void PopConvceController::setIOcontrollerSize()
{
    // wrt inputs
...
void PopConvceController::initController()
{
    _nbHiddenLayers = PopConvceSharedData::gNbHiddenLayers;
    _nbNeuronsPerHiddenLayer = new std::vector<unsigned int>(_nbHiddenLayers);
    for(unsigned int i = 0; i < _nbHiddenLayers; i++)
        (*_nbNeuronsPerHiddenLayer)[i] = PopConvceSharedData::gNbNeuronsPerHiddenLayer;
    ...
    {
        _currentGenome.push_back((double)(rand()%PopConvceSharedData::gNeuronWeightRange)/(PopConvceSharedData::gNeuronWeightRange/2)-1.0); // weights: random init between -1 and +1
    }
...
void PopConvceController::clearReservoir()
{
...
void PopConvceController::reset()
{
    initController();
}


void PopConvceController::mutateSigmaValue()
{
    float dice = float(rand()%100) / 100.0;
    
    if ( dice <= PopConvceSharedData::gProbaMutation )
    {
        dice = float(rand() %100) / 100.0;
        if ( dice < 0.5 )
        {
            _currentSigma = _currentSigma * ( 1 + PopConvceSharedData::gUpdateSigmaStep ); // increase sigma
            
            if (_currentSigma > PopConvceSharedData::gSigmaMax)
            {
                _currentSigma = PopConvceSharedData::gSigmaMax;
            }
        }
        else
        {
            _currentSigma = _currentSigma * ( 1 - PopConvceSharedData::gUpdateSigmaStep ); // decrease sigma
            
            if ( _currentSigma < PopConvceSharedData::gSigmaMin )
            {
                _currentSigma = PopConvceSharedData::gSigmaMin;
            }
        }
    }
}
...
void PopConvceController::broadcastGenome()
{
    // remarque \todo: limiting genome transmission is sensitive to sensor order. (but: assume ok)
    
    for( int i = 0 ; i < _wm->_cameraSensorsNb && ( PopConvceSharedData::gLimitGenomeTransmission == false || ( PopConvceSharedData::gLimitGenomeTransmission == true && _nbGenomeTransmission < PopConvceSharedData::gMaxNbGenomeTransmission ) ); i++)
    {
        ...
        if ( targetIndex >= gRobotIndexStartOffset && targetIndex < gRobotIndexStartOffset+gNbOfRobots )   // sensor ray bumped into a robot : communication is possible
        {
            ...
            PopConvceController* targetRobotController = dynamic_cast<PopConvceController*>(gWorld->getRobot(targetIndex)->getController());
...
void PopConvceController::performSelection() // called only if at least 1 genome was stored.
{
    switch ( PopConvceSharedData::gSelectionMethod )
    {
        case 0:
            selectRandomGenome();
            break;
        ...
        case 2:
            ...
            break;
        default:
            std::cerr << "[ERROR] unknown selection method (gSelectionMethod = " << PopConvceSharedData::gSelectionMethod << ")\n";
            exit(-1);
    }
}


void PopConvceController::loadNewGenome()
{
    if ( _wm->isAlive() || gEnergyRefill )  // ( gEnergyRefill == true ) enables revive
    {
        ...
        else
        {
            ...
            if ( _wm->isAlive() == true )
            {
                ...
                if ( PopConvceSharedData::gNotListeningStateDelay != 0 ) // ie. -1 (infinite,dead) or >0 (temporary,mute)
                {
                    _isListening = false;
...
void PopConvceController::logCurrentState()
{
    // Logging
...
double PopConvceController::getFitness()
{
    // nothing to do
 ...
 */
void PopConvceController::resetFitness()
{
    // nothing to do
    _wm->_fitnessValue = 0;
}

void PopConvceController::updateFitness()
{
    // nothing to do
    _wm->_fitnessValue = _wm->_desiredTranslationalValue;
}
