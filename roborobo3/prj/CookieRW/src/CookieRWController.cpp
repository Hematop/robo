...
CookieRWController::CookieRWController( RobotWorldModel *__wm ) : Controller ( __wm )
{
...
CookieRWController::~CookieRWController()
{
	// nothing to do.
}

void CookieRWController::reset()
{
	// nothing to do.
}

void CookieRWController::step()
{
    ...
    // listen to neighbors with weight 2, to oneself with weight 1, and add a noise of weight 1
    int consensus = 8 * _isAttracted + 2 * rand()%2 - 5;
    for(int i = 0; i < _wm->_cameraSensorsNb; i++)
    {
        ...
        if ( targetIndex >= gRobotIndexStartOffset && targetIndex < gRobotIndexStartOffset+gNbOfRobots )   // sensor ray bumped into a robot : communication is possible
        {
            ...
            CookieRWController* targetRobotController = dynamic_cast<CookieRWController*>(gWorld->getRobot(targetIndex)->getController());
            ...
            if ( ( (double)(rand()%100))/100.0 > 0.1 )
                consensus += 4 * targetRobotController->_isAttracted - 2;
        }
    }
    ...
    double y = avgCenterOfMass_Y - _wm->_yReal;
...
void CookieRWController::moveTowards(double x, double y){
    ...
    {
        double slack = ((double)(((rand()%360))-180)) / 18;
        double angle = atan(y/x) * 45 / atan(1) - _wm->_agentAbsoluteOrientation + slack;
        if (x<0) angle += 180;
    ...
    //     if (y > x)
    //         if (y > -x)
    //             _wm->_desiredRotationalVelocity = - _wm->_agentAbsoluteOrientation - 180;
    //         else
    //             _wm->_desiredRotationalVelocity = - _wm->_agentAbsoluteOrientation + 90;
    //     else
    //         if (y > -x)
    //             _wm->_desiredRotationalVelocity = - _wm->_agentAbsoluteOrientation - 90;
    //         else
    //             _wm->_desiredRotationalVelocity = - _wm->_agentAbsoluteOrientation;
    // 
}

void CookieRWController::monitorSensoryInformation()
{
